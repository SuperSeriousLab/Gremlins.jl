...
The test command is: `julia --project=<pkgdir> --code-coverage=user <test_file>`
...
function baseline_run(
    ...
    jl = _julia_exe()
    cmd = Cmd([jl, "--project=$pkgdir", "--code-coverage=user", test_path])
